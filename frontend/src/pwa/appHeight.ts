 ...
 *                 band beneath it.
 *
 * `window.innerHeight` tracks the *visible* viewport height reliably across
 * iOS versions, so we measure it in JS and expose it as `--app-h`. The shell
 * height chain (`html, body, #root`) consumes it; the `100%` fallback keeps
 * first-paint correct until this runs.
 *
